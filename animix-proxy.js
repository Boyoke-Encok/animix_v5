...
class Hivera {
  ...
  #get_user_agent() {
    ...
    console.log(`[Akun ${this.accountIndex + 1}] Buat agen pengguna...`.blue);
  ...
  async makeRequest(url, method, data = {}, retries = 1) {
    ...
    do {
      ...
      try {
        ...
        this.log(`Permintaan gagal: ${url} | ${error?.response?.data?.message || error.message} | mencoba lagi...`, "warning");
        success = false;
  ...
  async runAccount() {
    ...
    console.log(`=========Akun ${accountIndex + 1}| ${firstName + " " + lastName} | ${this.proxyIP} | Mulai nanti ${timesleep} Kedua...`.green);
    this.#set_headers();
...
async function main() {
  const queryIds = loadData("akun.txt");
