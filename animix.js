...
class Animix {
    ...
    do {
      try {
        ...
        if (error.response && error.response.status === 401) {
          this.log("Gagal masuk. Silakan masuk lagi.", "error");
  ...
  async handleMergePets() {
    ...
    while (moms.length > 0) {
      ...
      if (mom !== undefined && dad !== undefined) {
        ...
        if (mom !== nextMom) {
          this.log(`Ngewe ${mom} and ${nextMom}💕`);
          ...
          if (resMix.success) {
            ...
            this.log(`Crott..😘 Name: ${petInfo.name} | Star: ${petInfo.star} | Class: ${petInfo.class}`, "success");
          }
          moms.splice(momIndex, 1);
          moms.splice(momIndex, 1);
          await sleep(1);
        ...
        this.log("Belum jodohnya 💔.", "warning");
        break;
      }
    }
  }
  ...
  async handleMergePetsAdvantage() {
    ...
    if (matchingPairs.length < 1) {
      this.log("Tidak ada yang bisa di jodohkan 😢", "warning");
      return;
    }
  ...
  async processAccount() {
    ...
    if (!userData.success) {
      this.log("Login gagal , Lewati akun..", "error");
      return;
    }
...
async function wait(seconds) {
  for (let i = seconds; i > 0; i--) {
    process.stdout.write(`\r${colors.cyan(`[*] Tunggu ${Math.floor(i / 60)} menit ${i % 60} detik`)}`.padEnd(80));
    await new Promise((resolve) => setTimeout(resolve, 1000));
  }
  readline.cursorTo(process.stdout, 0);
  readline.clearLine(process.stdout, 0);
  console.log(`Mulai putaran baru...`);
}

async function main() {
  console.log(colors.yellow("BOT INI DI KEMBANGKAN OLEH BOYOKE ENCOK"));

  const { endpoint: hasIDAPI, message } = await checkBaseUrl();
  if (!hasIDAPI) return console.log(`Tidak dapat menemukan ID API, coba lagi nanti!`.red);
  console.log(`${message}`.yellow);

  const data = loadData("akun.txt");
  ...
  while (true) {
    for (let i = 0; i < data.length; i += maxThreads) {
      ...
      const promises = batch.map(async (initData, indexInBatch) => {
        ...
        console.log(`=========Akun ${accountIndex + 1}| ${firstName + " " + lastName}`.green);
        ...
        return timeout(client.processAccount(), 24 * 60 * 60 * 1000).catch((err) => {
          client.log(`Kesalahan pemrosesan akun: ${err.message}`, "error");
        });
      });
      await Promise.allSettled(promises);
      await new Promise((resolve) => setTimeout(resolve, 5000));
    }
    console.log(`[${new Date().toLocaleTimeString()}] all akun done | wait ${settings.TIME_SLEEP} menit=============`.magenta);
